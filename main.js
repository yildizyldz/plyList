...
// Şarkıyı ayarlama
const setSong = (arayIndex) => {
  let { name, link, artist, image } = songsList[arayIndex];
  audio.src = link;
...
// Şarkı bitince otomatik olarak bir sonraki şarkıya geçiş
audio.onended = () => {
  nextSong();
};

// Karıştırıcı butonuna tıklanıldığında
...
// Anlık şarkıda geçen toplam zamanı gösterme
audio.addEventListener("timeupdate", () => {
  currentTimeRef.innerText = timeFormatter(audio.currentTime);
});

// Progress bar'ı tıklanıldığında zamanı ayarlama
...
// Listeyi kapatma butonuna tıklanıldığında
closeButton.addEventListener("click", () => {
  playListContainer.classList.add("hide");
});

// Playlist butonuna tıklanıldığında playlist'i gösterme
playListButton.addEventListener("click", () => {
  playListContainer.classList.toggle("hide");
});

// Şarkıyı oynatmaya başlama
...
window.onload = () => {
  ...
  setSong(index);
  pauseAudio();
  initializePlaylist(); // Playlist başlatıldı
};
